...
// Links to horarios/ when timetables are available
...
// Fallback for cached pages that predate js/paleta.js
if (!window.VT_PALETA) {
...
// Joins multipart segments by nearest neighbor; gaps over GAP_MAX stay separate
const GAP_MAX = 150;
function distM(a, b) {
...
// Data files generated by tools/build-datos.mjs
...
let PARADAS_CRUDAS = [];
try {
  // Only routes are required; stops and schedules are optional
...
// Stops are identified and labeled by their QGIS fid (uid)
...
// Pane order: casing < lines < stops < focus
...
// Keep in sync with --color-ida / --color-vuelta in css/visor.css
...
// First/last service per line (data/horarios.json)
const TIPOS_DIA = [
  { clave: "habiles", etiqueta: "Días hábiles" },
  { clave: "sabado",  etiqueta: "Días sábado" },
  { clave: "domingo", etiqueta: "Días domingo" },
];
// Service day starts at this hour: 01:00 on Saturday still counts as a weekday
...
const RADIO_PARADA_LINEA = 20; // meters
...
function popupParada(p) {
  // null means not surveyed, which is not the same as false
...
chkParadas.addEventListener("change", (e) => {
  ...
  // By design: stops stay on after unfocusing
  estado.paradasAutoEncendida = false;
  aplicarFiltrosParadas();
});
...
// Route finder: origin + destination -> best line
const CAMINATA_MAX = 900;   // meters
const RECORRIDO_MIN = 250;  // meters
...
// Fallback: Georef API for corners outside the local survey
...
function armarAutocompletarBuscador(campo, input, lista) {
  ...
  let vigente = true; // Ignores late Georef responses
...
if (AVISOS_HABILITADO && AVISOS_ACTIVOS.length) {
  ...
  function abrirAvisoBurbuja() {
    ...
    elAvisoBurbuja.querySelector(".vt-aviso-cerrar").addEventListener("click", () => {
      cerrarAvisoBurbuja();
      try { sessionStorage.setItem(AVISOS_DESCARTADOS_KEY, "1"); } catch (e) {}
    });
  }
  ...
  try { avisosYaDescartados = sessionStorage.getItem(AVISOS_DESCARTADOS_KEY) === "1"; } catch (e) {}
...
(function lineaDesdeURL() {
  let pedida = null;
  try { pedida = new URLSearchParams(location.search).get("linea"); } catch (e) {}
  const id = String(pedida || "").trim().toUpperCase();
  if (id && lineaPorId.has(id)) enfocar(id);
})();

// Links to horarios/ once data/frecuencias.json loads
...
if (FRECUENCIAS_HABILITADO) {
  traerJSON(RUTA_FRECUENCIAS).then(sumarAccesosHorarios).catch(() => {});
}
...
function finalizarTour() {
  cerrarTour();
  try { localStorage.setItem(TOUR_COMPLETADO_KEY, "1"); } catch (e) {}
  pulsarAyuda();
}
...
if (TOUR_HABILITADO) {
  (function ofrecerTourSiCorresponde() {
    ...
    try { localStorage.setItem(TOUR_OFRECIDAS_KEY, String(ofrecidas + 1)); } catch (e) {}
